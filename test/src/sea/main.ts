...
(async function () {
  ...
  sceneView.addEventListener('draw', function (this: dom.RElement, evt: REvent) {
    evt.preventDefault();
    bindGroup.setValue('uniforms', new Vector4(viewer.device.frameInfo.elapsedOverall * 0.001, 0, viewer.device.getDrawingBufferWidth(), viewer.device.getDrawingBufferHeight()));
