...
type VectorType = base.Vector2 | base.Vector3 | base.Vector4;
...
type MatrixType = base.Matrix3x3 | base.Matrix4x4;
...
export function testQuaternion() {
  function randQuaternion() {
    const q = new base.Quaternion(rand(), rand(), rand(), rand());
    return q.inplaceNormalize();
  }
  (function testConstructor() {
    const q1 = new base.Quaternion();
    assert(q1.x === 0 && q1.y === 0 && q1.z === 0 && q1.w === 0, 'default constructor failed');
    const x = rand();
    const y = rand();
    const z = rand();
    const w = rand();
    const q2 = new base.Quaternion(x, y, z, w);
    assert(Math.abs(q2.x - x) < 0.01 && Math.abs(q2.y - y) < 0.01 && Math.abs(q2.z - z) < 0.01 && Math.abs(q2.w - w) < 0.01, 'assign constructor failed');
    const q3 = new base.Quaternion([x, y, z, w]);
    assert(q3.x === q2.x && q3.y === q2.y && q3.z === q2.z && q3.w === q2.w, 'array constructor failed');
    const axis = new base.Vector3(-0.10452544552113772, 0.43864725489574524, -0.8925598114474093); //new chaospace.Vector3(rand(), rand(), rand()).inplaceNormalize();
    const angle = 3.1390559093244788; //rand (0, Math.PI);
    const axisAngle = new base.Quaternion(base.Matrix3x3.rotation(axis, angle)).getAxisAngle();
    assert(axisAngle.xyz().equalsTo(axis) && base.numberEquals(axisAngle.w, angle), 'chaospace.Matrix3x3 constructor failed');
    const axisAngle2 = new base.Quaternion(base.Matrix4x4.rotation(axis, angle)).getAxisAngle();
    assert(axisAngle2.xyz().equalsTo(axis) && base.numberEquals(axisAngle2.w, angle), 'chaospace.Matrix4x4 constructor failed');
  })();
  (function testNormalize() {
    const q1 = new base.Quaternion(rand(), rand(), rand(), rand()).inplaceNormalize();
    assert(base.numberEquals(q1.magnitude, 1), 'normalize failed');
  })();
  (function testVetorToVector() {
    const v1 = new base.Vector3(rand(), rand(), rand()).inplaceNormalize();
    const v2 = new base.Vector3(rand(), rand(), rand()).inplaceNormalize();
    const q1 = base.Quaternion.unitVectorToUnitVector(v1, v2);
    const v3 = q1.transform(v1);
    assert(v2.equalsTo(v3), 'vector to vector rotation failed');
  })();
  (function testAngleBetween() {
    const q1 = new base.Quaternion(rand(), rand(), rand(), rand()).inplaceNormalize();
    const q2 = new base.Quaternion(rand(), rand(), rand(), rand()).inplaceNormalize();
    const angle = base.Quaternion.angleBetween(q1, q2);
    const axis1 = q1.getAxisAngle().xyz();
    const axis2 = q2.getAxisAngle().xyz();
    const angle2 = Math.acos(base.Vector3.dot(axis1, axis2));
  ...
  (function testEulerAngle() {
    ...
    const matX = base.Matrix3x3.rotationX(angle1);
    const matY = base.Matrix3x3.rotationY(angle2);
    const matZ = base.Matrix3x3.rotationZ(angle3);
    const testMat = new base.Matrix3x3();
    const rotMatrixXYZ = base.Matrix3x3.identity().multiplyRightAffine(matX).multiplyRightAffine(matY).multiplyRightAffine(matZ);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'XYZ').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixXYZ), 'euler angle XYZ failed');
    const rotMatrixXZY = base.Matrix3x3.identity().multiplyRightAffine(matX).multiplyRightAffine(matZ).multiplyRightAffine(matY);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'XZY').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixXZY), 'euler angle XZY failed');
    const rotMatrixYXZ = base.Matrix3x3.identity().multiplyRightAffine(matY).multiplyRightAffine(matX).multiplyRightAffine(matZ);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'YXZ').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixYXZ), 'euler angle YXZ failed');
    const rotMatrixYZX = base.Matrix3x3.identity().multiplyRightAffine(matY).multiplyRightAffine(matZ).multiplyRightAffine(matX);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'YZX').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixYZX), 'euler angle YZX failed');
    const rotMatrixZXY = base.Matrix3x3.identity().multiplyRightAffine(matZ).multiplyRightAffine(matX).multiplyRightAffine(matY);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'ZXY').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixZXY), 'euler angle ZXY failed');
    const rotMatrixZYX = base.Matrix3x3.identity().multiplyRightAffine(matZ).multiplyRightAffine(matY).multiplyRightAffine(matX);
    base.Quaternion.fromEulerAngle(angle1, angle2, angle3, 'ZYX').toMatrix(testMat);
    assert(testMat.equalsTo(rotMatrixZYX), 'euler angle ZYX failed');
  })();
  (function testSlerp() {
    const axis = new base.Vector3(rand(), rand(), rand()).inplaceNormalize();
    const a1 = rand(0, Math.PI / 2);
    const a2 = a1 + Math.PI / 2;
    const t = rand(0, 1);
    const a3 = a1 * (1 - t) + a2 * t;
    const q1 = base.Quaternion.fromAxisAngle(axis, a1);
    const q2 = base.Quaternion.fromAxisAngle(axis, a2);
    const q3 = base.Quaternion.slerp(q1, q2, t);
    const q4 = base.Quaternion.fromAxisAngle(axis, a3);
...
export function testMatrixType(c: MatrixConstructor, rows: number, cols: number) {
  ...
  if (c.translation) {
    (function testTranslation() {
      ...
      if (rows === 4 && cols === 4) {
        const m1 = randMatrix() as base.Matrix4x4;
        const t1 = { x: rand(), y: rand(), z: rand() };
        const m2 = m1.multiplyRight(base.Matrix4x4.translation(t1));
        m1.translateRight(t1);
        assert(m1.equalsTo(m2), 'translateRight test failed');
        const m3 = randMatrix() as base.Matrix4x4;
        const t2 = { x: rand(), y: rand(), z: rand() };
        const m4 = m3.multiplyLeft(base.Matrix4x4.translation(t2));
        m3.translateLeft(t2);
  ...
  (function testSaling() {
    ...
    if (rows === 4 && cols === 4) {
      ...
      const m2 = m1.multiplyRight(base.Matrix4x4.scaling(s1));
      m1.scaleRight(s1);
      assert(m1.equalsTo(m2), 'scaleRight test failed');
      const m3 = randMatrix() as base.Matrix4x4;
      const s2 = { x: rand(), y: rand(), z: rand() };
      const m4 = m3.multiplyLeft(base.Matrix4x4.scaling(s2));
      m3.scaleLeft(s2);
  ...
  (function testRotation() {
    ...
    if (rows === 4 && cols === 4) {
      const m1 = randMatrix() as base.Matrix4x4;
      const r1 = base.Matrix4x4.rotation(new base.Vector3(rand(), rand(), rand()).inplaceNormalize(), rand(0, Math.PI * 2));
      const m2 = m1.multiplyRight(r1);
      m1.rotateRight(r1);
      assert(m1.equalsTo(m2), 'rotateRight test failed');
      const m3 = randMatrix() as base.Matrix4x4;
      const r2 = base.Matrix4x4.rotation(new base.Vector3(rand(), rand(), rand()).inplaceNormalize(), rand(0, Math.PI * 2));
  ...
  if (rows === 4 && cols === 4) {
    (function testDecompose() {
      const eye = new base.Vector3(rand(), rand(), rand()).inplaceNormalize().scaleBy(100);
      const target = new base.Vector3(rand(), rand(), rand()).inplaceNormalize().scaleBy(100);
      const up = new base.Vector3(rand(), rand(), rand()).inplaceNormalize();
      const matrix = base.Matrix4x4.lookAt(eye, target, up);
      const t = new base.Vector3();
      const s = new base.Vector3();
      const r = new base.Matrix4x4();
      matrix.decompose(s, r, t);
      const matrix2 = base.Matrix4x4.translation(t).multiplyRightAffine(r).multiplyRightAffine(base.Matrix4x4.scaling(s));
      assert(matrix.equalsTo(matrix2), 'decompose test failed');
      const eye2 = new base.Vector3();
      const target2 = new base.Vector3();
      const up2 = new base.Vector3();
    ...
    (function testProjection() {
      ...
      const m1 = base.Matrix4x4.perspective(fovY, aspect, near, far);
      const m2 = base.Matrix4x4.frustum(left, right, bottom, top, near, far);
      const vmin = new base.Vector4(-1, -1, -1, 1);
      const vmax = new base.Vector4(1, 1, 1, 1);
      assert(m1.equalsTo(m2), 'projection test failed');
      const vx1 = m2.transformPoint(new base.Vector4(left, bottom, -near, 1));
      vx1.scaleBy(1 / vx1.w);
      const vx2 = m2.transformPoint(new base.Vector4(right * far / near, top * far / near, -far, 1));
      vx2.scaleBy(1 / vx2.w);
      ...
      const v = [
        new base.Vector3(left, bottom, -near),
        new base.Vector3(right, bottom, -near),
        new base.Vector3(right, top, -near),
        new base.Vector3(left, top, -near),
        new base.Vector3(leftFar, bottomFar, -far),
        new base.Vector3(rightFar, bottomFar, -far),
        new base.Vector3(rightFar, topFar, -far),
        new base.Vector3(leftFar, topFar, -far),
      ];
      const v2 = [
        new base.Vector4(-1, -1, -1, 1),
        new base.Vector4(1, -1, -1, 1),
        new base.Vector4(1, 1, -1, 1),
        new base.Vector4(-1, 1, -1, 1),
        new base.Vector4(-1, -1, 1, 1),
      ...
      const m3 = base.Matrix4x4.ortho(left, right, bottom, top, near, far);
      const v3 = [
        new base.Vector3(left, bottom, -near),
        new base.Vector3(right, bottom, -near),
        new base.Vector3(right, top, -near),
        new base.Vector3(left, top, -near),
        new base.Vector3(left, bottom, -far),
        new base.Vector3(right, bottom, -far),
        new base.Vector3(right, top, -far),
        new base.Vector3(left, top, -far),
      ];
      const v4 = [
        new base.Vector4(-1, -1, -1, 1),
        new base.Vector4(1, -1, -1, 1),
...
export function testXForm() {
  function multiplyMatrices(matrices: base.Matrix4x4[], out: base.Matrix4x4[], outInv: base.Matrix4x4[]) {
    out[0] = new base.Matrix4x4(matrices[0]);
    outInv[0] = base.Matrix4x4.inverseAffine(out[0]);
    for (let i = 1; i < matrices.length; i++) {
      out[i] = base.Matrix4x4.multiply(out[i - 1], matrices[i]);
      outInv[i] = base.Matrix4x4.inverseAffine(out[i]);
    }
  }
  function combinedMatrices(matrices: base.Matrix4x4[], matricesInv: base.Matrix4x4[], start: number, end: number) {
    if (start === 0) {
      return matrices[end];
    } else {
      return base.Matrix4x4.multiplyAffine(matricesInv[start - 1], matrices[end]);
    }
  }
  ...
  const count = 10;
  const xformArray = Array<device.XForm>(count);
  const matrixArray = Array<base.Matrix4x4>(count);
  const mulArray = Array<base.Matrix4x4>(count);
  const invArray = Array<base.Matrix4x4>(count);
  for (let i = 0; i < count; i++) {
    xformArray[i] = new device.XForm(null, xformArray[i - 1]);
    matrixArray[i] = base.Matrix4x4.identity();
  }
  const indices = randomIndices(0, count - 1, 50);
  indices[0] = 0;
  for (const index of indices) {
    const t = new base.Vector3(rand(-1000, 1000), rand(-1000, 1000), rand(-1000, 1000));
    const r = new base.Quaternion(rand(-1, 1), rand(-1, 1), rand(-1, 1), rand(-1, 1)).inplaceNormalize();
    const s = new base.Vector3(rand(1, 10), rand(1, 10), rand(1, 10));
    if (index === 0) {
      matrixArray[index].assign(base.Matrix4x4.translation(t).rotateRight(r).getArray());
      xformArray[index].position = t;
      xformArray[index].rotation = r;
    } else {
      matrixArray[index].assign(base.Matrix4x4.translation(t).rotateRight(r).scaleRight(s).getArray());
