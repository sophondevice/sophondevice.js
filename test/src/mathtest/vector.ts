import { Vector2, Vector3, Vector4, Quaternion, Matrix3x3, Matrix4x4, numberEquals } from '@sophon/base';
import { XForm } from '@sophon/scene';
...
export function testXForm() {
  function multiplyMatrices(matrices: Matrix4x4[], out: Matrix4x4[], outInv: Matrix4x4[]) {
    ...
    outInv[0] = Matrix4x4.inverseAffine(out[0]);
    for (let i = 1; i < matrices.length; i++) {
      out[i] = Matrix4x4.multiply(out[i - 1], matrices[i]);
      outInv[i] = Matrix4x4.inverseAffine(out[i]);
    }
  }
  ...
  for (let i = 0; i < count; i++) {
    xformArray[i] = new XForm(null, xformArray[i - 1]);
  ...
  for (const index of indices) {
    ...
    if (index === 0) {
      matrixArray[index].assign(Matrix4x4.translation(t).rotateRight(r).getArray());
      xformArray[index].position = t;
      xformArray[index].rotation = r;
    } else {
      matrixArray[index].assign(Matrix4x4.translation(t).rotateRight(r).scaleRight(s).getArray());
