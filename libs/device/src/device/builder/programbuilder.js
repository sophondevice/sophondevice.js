...
import { PBAddressSpace, PBArrayTypeInfo, PBPointerTypeInfo, PBPrimitiveType, PBPrimitiveTypeInfo, PBSamplerAccessMode, PBStructTypeInfo, typeBool, typeF32, typeFrexpResult, typeFrexpResultVec2, typeFrexpResultVec3, typeFrexpResultVec4, typeI32, typeTex2D, typeTexCube, typeU32, typeVoid } from './types';
...
export function makeConstructor(typeFunc, elementType) {
...
export class PBScope extends Proxiable {
    $_variables;
    $_parentScope;
    $_AST;
    $_localScope;
    ...
    $_declareInternal(variable, init) {
        ...
        if (init === undefined || init === null) {
            return new AST.ASTDeclareVar(variable.$ast);
        }
        else {
            if (init instanceof PBShaderExp && init.$ast instanceof AST.ASTShaderExpConstructor && init.$ast.args.length === 0) {
                ...
                return new AST.ASTAssignment(new AST.ASTLValueDeclare(variable.$ast), init instanceof PBShaderExp ? init.$ast : init);
            }
        }
    }
...
export class PBGlobalScope extends PBScope {
    ...
    $internalFunction(name, params, isMain, body) {
        ...
        const astFunc = new AST.ASTFunction(name, params.map(param => param.$ast), isMain);
...
export class PBInsideFunctionScope extends PBScope {
    ...
    $for(counter, init, count, body) {
        ...
        const astFor = new AST.ASTRange(counter.$ast, initval, count instanceof PBShaderExp ? count.$ast : new AST.ASTScalar(count, initializerType), true);
...
export class PBDoWhileScope extends PBInsideFunctionScope {
    ...
    $while(condition) {
        this.$ast.condition = condition instanceof PBShaderExp ? condition.$ast : new AST.ASTScalar(condition, typeof condition === 'number' ? typeF32 : typeBool);
    }
}
...
let uidExp = 0;
export class PBShaderExp extends Proxiable {
    $uid;
    $str;
    $location;
    $typeinfo;
