...
export class PipelineCache {
  ...
  private createPrimitiveState(
    ...
    const rasterizerState =
      stateOverridden?.rasterizerState ||
      stateSet?.rasterizerState ||
  ...
  private createDepthStencilState(
    ...
    const stateOverridden = this._device.getRenderStatesOverridden();
    const depthState =
      stateOverridden?.depthState ||
      stateSet?.depthState ||
      (rs.WebGPUDepthState.defaultState as rs.WebGPUDepthState);
    const stencilState =
      stateOverridden?.stencilState ||
      stateSet?.stencilState ||
  ...
  private createColorTargetState(stateSet: WebGPURenderStateSet, format: GPUTextureFormat) {
    const stateOverridden = this._device.getRenderStatesOverridden();
    const blendingState =
      stateOverridden?.blendingState ||
      stateSet?.blendingState ||
      (rs.WebGPUBlendingState.defaultState as rs.WebGPUBlendingState);
    const colorState =
      stateOverridden?.colorState ||
      stateSet?.colorState ||
