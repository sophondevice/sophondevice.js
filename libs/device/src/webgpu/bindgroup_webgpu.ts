...
export class WebGPUBindGroup extends WebGPUObject<unknown> implements BindGroup {
      ...
      if (entry) {
        ...
        if (!t || t[0] !== value || t[1] !== view) {
          ...
          if (entry.texture?.autoBindSampler) {
            ...
            this._resources[entry.texture.autoBindSampler] = (
              value.getDefaultSampler(false) as WebGPUTextureSampler
            ).object;
          }
          if (entry.texture?.autoBindSamplerComparison) {
            ...
            this._resources[entry.texture.autoBindSamplerComparison] = (
              value.getDefaultSampler(true) as WebGPUTextureSampler
            ).object;
          }
          this._bindGroup = null;
        }
  ...
  getResource(
    name: string
  ): StructuredBuffer | WebGPUTextureVideo | [WebGPUBaseTexture, GPUTextureView] | GPUSampler {
