import { PBInsideFunctionScope, PBShaderExp } from '@sophon/device';
...
import {
  ...
  RENDER_PASS_TYPE_MULTI_FORWARD
} from '../values';
...
export function forwardComputeLighting(
  ...
  if (ctx.renderPass.getRenderPassType() !== RENDER_PASS_TYPE_FORWARD) {
    throw new Error(
      `forwardComputeLighting() failed: invalid render pass type: ${ctx.renderPass.getRenderPassType()}`
    );
  }
      ...
      pb.globalScope.$function(
        ...
        function () {
          this.$l.dist = pb.distance(
            ...
            pb.queryGlobal(ShaderLib.USAGE_WORLD_POSITION).xyz
          );
    ...
    if (!pb.getFunction(funcNameIllumCascadedShadowLight)) {
      pb.globalScope.$function(
        ...
        function () {
          ...
          this.shadowVertex = pb.vec4(
            pb.dot(
              this.global.light.lightParams.at(pb.add(pb.mul(this.split, 4), 7)),
              this.$query(ShaderLib.USAGE_WORLD_POSITION)
            ),
            pb.dot(
              this.global.light.lightParams.at(pb.add(pb.mul(this.split, 4), 8)),
              this.$query(ShaderLib.USAGE_WORLD_POSITION)
            ),
            pb.dot(
              this.global.light.lightParams.at(pb.add(pb.mul(this.split, 4), 9)),
              this.$query(ShaderLib.USAGE_WORLD_POSITION)
            ),
            pb.dot(
              this.global.light.lightParams.at(pb.add(pb.mul(this.split, 4), 10)),
              this.$query(ShaderLib.USAGE_WORLD_POSITION)
            )
          );
  ...
  if (!pb.getFunction(funcNameComputeLighting)) {
    pb.globalScope.$function(funcNameComputeLighting, [], function () {
      ...
      this.$l.surfaceData = lm.getSurfaceData(
        this,
        env,
        worldPosition,
        worldNormal,
...
export function forwardComputeLightingMultiPass(
  scope: PBInsideFunctionScope,
  lm: LightModel,
  ctx: DrawContext
): PBShaderExp {
            ...
            if (ctx.shadowMapper.numShadowCascades > 1) {
              ...
              this.$for(pb.int('cascade'), 0, 4, function () {
                this.$if(pb.equal(this.cascade, this.split), function () {
                  this.shadowVertex = pb.vec4(
                    pb.dot(
                      this.global.light.shadowMatrix.at(pb.mul(this.cascade, 4)),
                      this.$query(ShaderLib.USAGE_WORLD_POSITION)
                    ),
                    pb.dot(
                      this.global.light.shadowMatrix.at(pb.add(pb.mul(this.cascade, 4), 1)),
                      this.$query(ShaderLib.USAGE_WORLD_POSITION)
                    ),
                    pb.dot(
                      this.global.light.shadowMatrix.at(pb.add(pb.mul(this.cascade, 4), 2)),
                      this.$query(ShaderLib.USAGE_WORLD_POSITION)
                    ),
                    pb.dot(
                      this.global.light.shadowMatrix.at(pb.add(pb.mul(this.cascade, 4), 3)),
                      this.$query(ShaderLib.USAGE_WORLD_POSITION)
                    )
                  );
                  this.$break();
              ...
              this.$l.shadow = ctx.shadowMapper.computeShadowCSM(
                this,
                this.shadowVertex,
                this.NdotL,
                this.split
