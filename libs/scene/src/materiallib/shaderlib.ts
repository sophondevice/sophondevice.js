import { ProgramBuilder, PBInsideFunctionScope, PBPrimitiveType, PBShaderExp } from '@sophon/device';
...
export class ShaderLib {
  ...
  ftransform(inputPos?: PBShaderExp): PBShaderExp {
    ...
    const pos = inputPos || pb.globalScope.$getVertexAttrib('position');
    if (!pos) {
  ...
  getSkinMatrix(): PBShaderExp {
    ...
    if (!pb.getFunction(ShaderLib.funcNameGetSkinningMatrix)) {
      pb.globalScope.$function(ShaderLib.funcNameGetSkinningMatrix, [], function () {
        ...
        const blendIndices = pb.globalScope.$getVertexAttrib('blendIndices');
