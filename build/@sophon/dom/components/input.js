/** sophon base library */
import { RPrimitiveBatchList, RRectPrimitive } from '../primitive.js';
import { RText } from './text.js';
import { RElement } from '../element.js';
import { RElementLayoutEvent, RFocusEvent, RMouseEvent, RTextEvent, RKeyEvent, RChangeEvent } from '../events.js';

class Input extends RElement {
    _hiddenInput;
    _selectionStart;
    _selectionEnd;
    _drawCursor;
    ...
    constructor(uiscene) {
        ...
        this.addDefaultEventListener(RFocusEvent.NAME_FOCUS, function () {
            if (this.type === 'text') {
                this._hiddenInput.focus();
                this._restartCursorTimer();
            }
        });
        this.addDefaultEventListener(RFocusEvent.NAME_BLUR, function () {
            if (this.type === 'text') {
        ...
        this.addDefaultEventListener(RMouseEvent.NAME_MOUSEDOWN, function (e) {
            const data = e;
            if (data.button === 0) {
                if (this.type === 'text') {
                    this._hiddenInput.focus();
                    const loc = this._text.measureTextLocation(data.offsetX - this.getClientRect().x - this._text.getRect().x, data.offsetY - this.getClientRect().y - this._text.getRect().y);
                    if (loc) {
        ...
        this.addDefaultEventListener(RTextEvent.NAME_CONTENT_CHANGE, function () {
        ...
        this._hiddenInput.addEventListener('keydown', function (evt) {
            ...
            const keyEvt = new RKeyEvent(RKeyEvent.NAME_KEYDOWN, evt.code, evt.key, evt.repeat, evt.ctrlKey, evt.shiftKey, evt.altKey, evt.metaKey);
            that.dispatchEvent(keyEvt);
        });
        this._hiddenInput.addEventListener('keyup', function (evt) {
            const keyEvt = new RKeyEvent(RKeyEvent.NAME_KEYUP, evt.code, evt.key, evt.repeat, evt.ctrlKey, evt.shiftKey, evt.altKey, evt.metaKey);
            that.dispatchEvent(keyEvt);
        });
        this._hiddenInput.addEventListener('keypress', function (evt) {
            const keyEvt = new RKeyEvent(RKeyEvent.NAME_KEYPRESS, evt.code, evt.key, evt.repeat, evt.ctrlKey, evt.shiftKey, evt.altKey, evt.metaKey);
            that.dispatchEvent(keyEvt);
        });
    ...
    _setHiddenInputSelection(start, end) {
        if (this._hiddenInput.setSelectionRange) {
            ...
            const ieInput = this._hiddenInput;
    ...
    _setHiddenInputCaretPosition(pos) {
    ...
    _updateCursorVertices() {
        ...
        if (clipper) {
            ...
            this._cursorBatch.addPrimitive(new RRectPrimitive(x, this.getClientRect().y - 2, 1, this.getClientRect().height, 0, 0, 0, 0), clipper, null, this._getCachedFontColor());
        }
    }
    _stopCursorTimer() {
