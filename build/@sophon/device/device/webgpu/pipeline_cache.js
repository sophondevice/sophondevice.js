...
class PipelineCache {
    ...
    createPrimitiveState(vertexData, stateSet, primitiveType) {
        ...
        const rasterizerState = stateOverridden?.rasterizerState || stateSet?.rasterizerState || WebGPURasterizerState.defaultState;
    ...
    createDepthStencilState(depthFormat, stateSet) {
        ...
        const stateOverridden = this._device.getRenderStatesOverridden();
        const depthState = stateOverridden?.depthState || stateSet?.depthState || WebGPUDepthState.defaultState;
        const stencilState = stateOverridden?.stencilState || stateSet?.stencilState || WebGPUStencilState.defaultState;
    ...
    createColorTargetState(stateSet, format) {
        const stateOverridden = this._device.getRenderStatesOverridden();
        const blendingState = stateOverridden?.blendingState || stateSet?.blendingState || WebGPUBlendingState.defaultState;
        const colorState = stateOverridden?.colorState || stateSet?.colorState || WebGPUColorState.defaultState;
