...
class WebGPUBindGroup extends WebGPUObject {
    ...
    setTextureView(name, value, level, face, mipCount) {
        if (!value) {
            ...
            if (entry) {
                ...
                if (!t || t[0] !== value || t[1] !== view) {
                    this._resources[name] = [value, view];
                    if (entry.texture?.autoBindSampler) {
                        ...
                        this._resources[entry.texture.autoBindSampler] = value.getDefaultSampler(false).object;
                    }
                    if (entry.texture?.autoBindSamplerComparison) {
                        ...
                        this._resources[entry.texture.autoBindSamplerComparison] = value.getDefaultSampler(true).object;
                    }
                    this._bindGroup = null;
                }
            }
    ...
    _getBuffer(name, usage, nocreate = false) {
        for (const entry of this._layout.entries) {
    ...
    _create() {
        ...
        let resourceOk = true;
        for (const entry of this._layout.entries) {
            const ge = { binding: entry.binding };
            if (entry.buffer) {
