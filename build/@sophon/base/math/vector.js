/** sophon base library */
import { numberEquals, formatNumber, numberClamp } from './misc.js';
...
class VectorBase extends IterableWrapper {
    _doCallback;
    _changeCallback;
    constructor(nOrArray) {
        super(nOrArray);
...
class Quaternion extends VectorBase {
    ...
    transform(v, result) {
        ...
        const wy = this.w * y;
        const wz = this.w * z;
        return result.set((1 - yy - zz) * v.x + (xy - wz) * v.y + (xz + wy) * v.z, (xy + wz) * v.x + (1 - xx - zz) * v.y + (yz - wx) * v.z, (xz - wy) * v.x + (yz + wx) * v.y + (1 - xx - yy) * v.z);
    }
    addBy(other) {
    ...
    static magnitudeSq(q) {
        return q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w;
    }
    static add(q, other, result) {
        result = result || q;
    ...
    static slerp(a, b, t, result) {
        ...
        return result.set(ax * ratioA + bx * ratioB, ay * ratioA + by * ratioB, az * ratioA + bz * ratioB, aw * ratioA + bw * ratioB);
    }
    ...
    static fromRotationMatrix(matrix, result) {
        ...
        if (trace > 0) {
            ...
            result.set((matrix.m21 - matrix.m12) * s, (matrix.m02 - matrix.m20) * s, (matrix.m10 - matrix.m01) * s, 0.25 / s);
        }
        else if (matrix.m00 > matrix.m11 && matrix.m00 > matrix.m22) {
            s = 2 * Math.sqrt(1 + matrix.m00 - matrix.m11 - matrix.m22);
            result.set(0.25 * s, (matrix.m01 + matrix.m10) / s, (matrix.m02 + matrix.m20) / s, (matrix.m21 - matrix.m12) / s);
        }
        else if (matrix.m11 > matrix.m22) {
            s = 2 * Math.sqrt(1 - matrix.m00 + matrix.m11 - matrix.m22);
            result.set((matrix.m10 + matrix.m01) / s, 0.25 * s, (matrix.m21 + matrix.m12) / s, (matrix.m02 - matrix.m20) / s);
        }
        else {
            s = 2 * Math.sqrt(1 - matrix.m00 - matrix.m11 + matrix.m22);
            result.set((matrix.m02 + matrix.m20) / s, (matrix.m12 + matrix.m21) / s, 0.25 * s, (matrix.m10 - matrix.m01) / s);
        }
        return result;
    }
}
class Matrix3x3 extends VectorBase {
    constructor(quatOrArray) {
    ...
    static inverseAffine(matrix, result) {
    ...
    transform(point, result) {
        ...
        return result.set(v[0] * px + v[3] * py + v[6] * pz, v[1] * px + v[4] * py + v[7] * pz, v[2] * px + v[5] * py + v[8] * pz);
    }
...
class Matrix4x4 extends VectorBase {
    ...
    getRow(row, result) {
        return (result || new Vector4()).set(this._v[row * 4], this._v[row * 4 + 1], this._v[row * 4 + 2], this._v[row * 4 + 3]);
    }
    getRow3(row, result) {
    ...
    static ortho(left, right, bottom, top, znear, zfar, result) {
        result = result || new Matrix4x4();
        const v = result.getArray();
        v[0] = 2 / (right - left);
        v[1] = 0;
    ...
    static perspectiveZ(fovY, aspect, znear, zfar, result) {
        const h = znear * Math.tan(fovY * 0.5);
        const w = h * aspect;
    ...
    static obliquePerspective(perspectiveMatrix, nearPlane) {
        ...
        const q = new Vector4(((nearPlane.x > 0 ? 1 : nearPlane.x < 0 ? -1 : 0) + perspectiveMatrix.m20) / perspectiveMatrix.m00, ((nearPlane.y > 0 ? 1 : nearPlane.y < 0 ? -1 : 0) + perspectiveMatrix.m21) / perspectiveMatrix.m11, -1, (1 + perspectiveMatrix.m22) / perspectiveMatrix.m32);
    ...
    static frustumZ(left, right, bottom, top, znear, zfar, result) {
        result = result || new Matrix4x4();
        const v = result.getArray();
        const dx = right - left;
        const dy = top - bottom;
    ...
    static lookAtCubeFace(face, result) {
        switch (face) {
            case CubeFace.PX:
    ...
    transformPoint(point, result) {
        ...
        const v = this.getArray();
        const px = point.x, py = point.y, pz = point.z;
        return result.set(v[0] * px + v[4] * py + v[8] * pz + v[12], v[1] * px + v[5] * py + v[9] * pz + v[13], v[2] * px + v[6] * py + v[10] * pz + v[14], v[3] * px + v[7] * py + v[11] * pz + v[15]);
    }
    transformPointAffine(point, result) {
        result = result || new Vector3();
        const v = this.getArray();
        ...
        const py = point.y;
        const pz = point.z;
        return result.set(v[0] * px + v[4] * py + v[8] * pz + v[12], v[1] * px + v[5] * py + v[9] * pz + v[13], v[2] * px + v[6] * py + v[10] * pz + v[14]);
    }
    transformVector(vec, result) {
        result = result || new Vector4();
        const v = this.getArray();
        const vx = vec.x, vy = vec.y, vz = vec.z;
        return result.set(v[0] * vx + v[4] * vy + v[8] * vz, v[1] * vx + v[5] * vy + v[9] * vz, v[2] * vx + v[6] * vy + v[10] * vz, v[3] * vx + v[7] * vy + v[11] * vz);
    }
    transformVectorAffine(vec, result) {
        result = result || new Vector3();
        const v = this.getArray();
        const vx = vec.x, vy = vec.y, vz = vec.z;
        return result.set(v[0] * vx + v[4] * vy + v[8] * vz, v[1] * vx + v[5] * vy + v[9] * vz, v[2] * vx + v[6] * vy + v[10] * vz);
    }
    transform(vec, result) {
        result = result || new Vector4();
        const v = this.getArray();
        const vx = vec.x, vy = vec.y, vz = vec.z, vw = vec.w;
        return result.set(v[0] * vx + v[4] * vy + v[8] * vz + v[12] * vw, v[1] * vx + v[5] * vy + v[9] * vz + v[13] * vw, v[2] * vx + v[6] * vy + v[10] * vz + v[14] * vw, v[3] * vx + v[7] * vy + v[11] * vz + v[15] * vw);
    }
    transformAffine(vec, result) {
        result = result || new Vector4();
        const v = this.getArray();
        const vx = vec.x, vy = vec.y, vz = vec.z, vw = vec.w;
        return result.set(v[0] * vx + v[4] * vy + v[8] * vz + v[12] * vw, v[1] * vx + v[5] * vy + v[9] * vz + v[13] * vw, v[2] * vx + v[6] * vy + v[10] * vz + v[14] * vw, vec.w);
    }
    det() {
        const v = this._v;
        const m00 = v[0], m01 = v[1], m02 = v[2], m03 = v[3];
        const m10 = v[4], m11 = v[5], m12 = v[6], m13 = v[7];
        const m20 = v[8], m21 = v[9], m22 = v[10], m23 = v[11];
        const m30 = v[12], m31 = v[13], m32 = v[14], m33 = v[15];
...
export { CubeFace, IterableWrapper, Matrix3x3, Matrix4x4, Quaternion, Vector2, Vector3, Vector4, VectorBase };
