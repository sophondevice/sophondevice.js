...
(async function() {
  ...
  const framebuffer = device.createFrameBuffer({
    colorAttachments: [{ texture: renderTargetColorTexture }],
    depthAttachment: { texture: renderTargetDepthTexture }
  });
  ...
  device.runLoop(device => {
    ...
    const projMatrix = Matrix4x4.perspective(1.5, device.getDrawingBufferWidth()/device.getDrawingBufferHeight(), 1, 50);
